...
input_excel = "../ReceivedData_2024/Helvetas/SUMS/District_Level_Spatial_Indicators_Data_Entry_Table_HELVETAS.xlsx"

grantee = 'Helvetas'
# Output folder
output_folder = f"../ProcessedData_2024/{grantee}/SUMS/supabase"
...
# # Print the list of sheets to be processed
...
for sheet_name, df in sheets_to_process.items():
    ...
    # Drop rows where 'district' column is 0
    ...
    df_json.to_csv(output_csv_path, index=False)
    print(f"CSV file for sheet '{sheet_name}' exported to {output_csv_path}")
...
def rename_file(old_file_name):
    # Remove spaces and dashes from the file name
    new_file_name = old_file_name.replace(" - ", "").replace(" ", "")
    # Insert an underscore before the year
    year_index = new_file_name.find("20")
    new_file_name = new_file_name[:year_index] + "_" + new_file_name[year_index:]
    return new_file_name

# Rename each CSV file
